...
Menu::Menu(string title, string inputText, vector<string> options, vector<string> list, bool validateInput,
           bool isToPrintList, int nColumns, bool acceptIntegers) : title(title), inputText(inputText),
                                                                    listToPrint(list), validateInput(validateInput),
                                                                    isToPrintList(isToPrintList), nColumns(nColumns),
...
Menu::Menu(string title, string inputText, vector<string> options, vector<string> list, vector<string> listToValidate,
           bool validateInput, bool isToPrintList, int nColumns, bool acceptIntegers) : title(title),
                                                                                        inputText(inputText),
                                                                                        listToPrint(list),
                                                                                        validateInput(validateInput),
                                                                                        isToPrintList(isToPrintList),
                                                                                        nColumns(nColumns),
                                                                                        acceptIntegers(acceptIntegers) {
    this->listIndex = 0;
    this->options = options;
...
bool Menu::inputIsInvalid(string input) {
    try {
        ...
        if (isToPrintList and listToPrint.size() > nLines * nColumns and inputInt >= 0 and
            inputInt <= options.size())
            return false;
        if (isToPrintList and listToPrint.size() > nLines * nColumns and inputInt >= 0 and listIndex > 0 and
            inputInt <= options.size() + 1)
            return false;
...
void Menu::render() {
    ...
    try {
        ...
        if (isToPrintList and listToPrint.size() > nLines * nColumns and inputInt == options.size() + 1 and
            listIndex > 0) {
            decreaseListIndex();
            render();
            return;
